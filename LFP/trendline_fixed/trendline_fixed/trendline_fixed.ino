...
void loop() {
  ...
  if (powerOn && setButtonState == LOW && setButtonPreviousState == HIGH) {
    if(!locked) {
      ...
      Serial.print("Ideal pressure SET to:");
      Serial.println(ideal_pressure);
      avgSet = 0.0;
      avgSetCount = 0;
    }
  }
...
void mainMode(float pressureInPSI){
  ...
  if(isFull(&sensorData)) {
    if(is_pressure_unacceptable(pressureInPSI) && !is_datapoint_an_outlier(&sensorData, pressureInPSI)){
...
bool is_datapoint_an_outlier(struct Queue* queue, float new_data){
  ...
  return residual_error > residual_error_threshold;
}
